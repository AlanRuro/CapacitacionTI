...
prepareStatement}.
     ...
     */
    public NamedParamStatement(Connection connection, String query) throws 
            SQLException {
...
method is non-private so JUnit code can
     ...
     * @param name  parameter name
     * @param value parameter value
     * @param length parameter length (byte array length)
     * @throws SQLException if an error occurred
     * @throws IllegalArgumentException if the parameter does not exist
    ...
    public void setBinaryStream(String name, InputStream value, int length) throws SQLException {
        int[] indexes=getIndexes(name);
        for(int i=0; i < indexes.length; i++) {
            statement.setBinaryStream(indexes[i], value, length); //Object(indexes[i], value);
        }
    }
    ...
    public void setBytes(String name, byte[] value) throws SQLException {
        int[] indexes=getIndexes(name);
        for(int i=0; i < indexes.length; i++) {
            statement.setBytes(indexes[i], value); //Object(indexes[i], value);
        }
    }
